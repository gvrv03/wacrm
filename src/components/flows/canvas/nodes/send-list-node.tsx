"use client";

import { memo } from "react";
import type { NodeProps } from "@xyflow/react";
import { Handle, Position } from "@xyflow/react";
import { BaseNode } from "./base-node";
import { truncate } from "../../lib/constants";
import type { FlowNodeData } from "../../lib/graph-utils";

function SendListNodeInner({ data, selected }: NodeProps) {
  const d = data as FlowNodeData;
  const text = typeof d.config.text === "string" ? d.config.text : "";
  const sections = (d.config.sections ?? []) as Array<{
    title?: string;
    rows: Array<{
      reply_id: string;
      title: string;
      next_node_key: string;
    }>;
  }>;

  let globalRowIdx = 0;
  const allRows: Array<{ title: string; handleId: string }> = [];
  for (const section of sections) {
    for (const row of section.rows ?? []) {
      allRows.push({ title: row.title, handleId: `row_${globalRowIdx}` });
      globalRowIdx++;
    }
  }

  return (
    <BaseNode
      nodeType="send_list"
      nodeKey={d.node_key}
      isEntry={d.isEntry}
      selected={selected}
      onDelete={d.onDelete as (() => void) | undefined}
      onDuplicate={d.onDuplicate as (() => void) | undefined}
      onEdit={d.onEdit as (() => void) | undefined}
      handles={
        allRows.length > 0 ? (
          <div className="relative pb-3">
            {allRows.map((row, idx) => (
              <div
                key={row.handleId}
                className="relative flex items-center justify-between border-t border-border/30 px-3 py-1"
              >
                <span className="text-[9px] text-foreground/70">
                  {row.title || `Row ${idx + 1}`}
                </span>
                <Handle
                  type="source"
                  position={Position.Right}
                  id={row.handleId}
                  className="!h-2.5 !w-2.5 !rounded-full !border-2 !border-white !bg-indigo-500/80 transition-colors hover:!bg-indigo-600 hover:!scale-110"
                  style={{ top: "50%", right: -5 }}
                />
              </div>
            ))}
          </div>
        ) : undefined
      }
    >
      {text ? (
        <p className="text-[10px] leading-relaxed text-foreground/80">
          {truncate(text, 50)}
        </p>
      ) : (
        <p className="text-[10px] italic text-muted-foreground/60">
          No body text
        </p>
      )}
      <p className="mt-1 text-[9px] text-muted-foreground">
        {allRows.length} row{allRows.length === 1 ? "" : "s"} across{" "}
        {sections.length} section{sections.length === 1 ? "" : "s"}
      </p>
    </BaseNode>
  );
}

export const SendListNode = memo(SendListNodeInner);
