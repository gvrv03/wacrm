...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  send_chatbot_reply: { label: "Send Bot Reply", icon: MessageSquare, border: "border-l-primary" },
}
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case "send_chatbot_reply":
      return (
        <ChatbotReplyPicker
          value={(cfg.chatbot_reply_id as string) ?? ""}
          onChange={(id) => set({ chatbot_reply_id: id })}
        />
      )
    default:
      return null
...
// ------------------------------------------------------------
// Chatbot Reply Picker — fetches available bot replies for selection
// ------------------------------------------------------------

function ChatbotReplyPicker({
  value,
  onChange,
}: {
  value: string
  ...
  const [replies, setReplies] = useState<Array<{ id: string; name: string; reply_type: string }>>([])
  ...
  if (loading) {
    return <p className="text-xs text-muted-foreground">Loading bot replies...</p>
  }
  ...
  if (replies.length === 0) {
    return (
      <div className="space-y-2">
        <p className="text-xs text-muted-foreground">
          No bot replies found. Create one first in the Chat Bot section.
        </p>
        <a href="/chatbot/new" className="text-xs text-primary hover:underline">
          + Create Bot Reply
        </a>
      </div>
    )
  ...
  return (
    <FieldBlock label="Select Bot Reply">
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground focus:outline-none"
      >
        <option value="">— Select a bot reply —</option>
        {replies.map((r) => (
